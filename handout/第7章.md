...
### 公有继承(public)
- 继承的访问控制
  - 基类的```cpp public```和```cpp protected```成员：**访问属性在派生类中**保持不变；
  - 基类的```cpp private```成员：**不可直接访问**。
- 访问权限
  - 派生类中的成员函数：可以直接访问基类中的```cpp public```和```cpp protected```成员，但不能直接访问基类的```cpp private```成员；
  - 通过派生类的对象：只能访问```cpp public```成员。
